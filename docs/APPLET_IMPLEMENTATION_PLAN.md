...
**Success criteria (v1):**
- Streamlit app available under `python/src/sofa_resp_sim/web/applet_streamlit.py`.
...
**Deliverables**
- Create `python/src/sofa_resp_sim/web/` package.
...
- `python/src/sofa_resp_sim/resp_simulation.py` (reuse only; no model-logic changes expected).
- `python/src/sofa_resp_sim/web/view_model.py` (input schema + validation).
- `python/src/sofa_resp_sim/web/app_services.py` (run/sweep orchestration and comparison metrics).
- `python/src/sofa_resp_sim/web/reference.py` (reference loaders/parsers).
- `python/src/sofa_resp_sim/web/applet_streamlit.py` (UI composition).
