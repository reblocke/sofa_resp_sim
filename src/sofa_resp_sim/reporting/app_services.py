...
from ..core.resp_simulation import (
    SimulationConfig,
    SupportPolicy,
    run_parameter_sweep,
    run_replicates,
...
def build_simulation_config(request: AppletRunRequest) -> SimulationConfig:
    ...
    room_air, low_flow, hfnc, nippv = derive_support_thresholds(normalized.room_air_threshold)
...
def format_sweep_summary_for_export(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    remaining_columns = [column for column in augmented.columns if column not in ordered_columns]
...
def build_sweep_heatmap_frame(summary_df: pd.DataFrame, metric: str) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            "Sweep summary is missing required heatmap columns: " + ", ".join(sorted(missing))
        )
...
def build_uncertainty_table(
    ...
    ci_table["p_reference"] = [
        float(reference[f"p_sofa_{score}"]) for score in ci_table["score"].astype(int).tolist()
    ]
...
def _coerce_probability_series(values: pd.Series, name: str) -> pd.Series:
    ...
    if all(column in values.index for column in REQUIRED_PROBABILITY_COLUMNS):
        ...
        try:
            ordered = pd.Series(
                [index_lookup[str(score)] for score in range(5)],
